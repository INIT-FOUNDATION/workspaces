...
export const authService = {
  createClient: async (clientDetails: ClientDetails) => {
    try {
      ...
      loggerUtils.error(
        `authService :: createClient :: clientObj ${JSON.stringify(
          clientDetails
        )} :: ${error}`
      );
      throw error;
  ...
  clientExistsByName: async (clientName: string) => {
    try {
      ...
      loggerUtils.error(
        `authService :: clientExists by clientName ${clientName} :: ${error}`
      );
      throw error;
    }
  },
  clientExistsById: async (clientId: string): Promise<boolean> => {
    try {
      ...
      loggerUtils.error(
        `authService :: clientExists by client Id ${clientId} :: ${error}`
      );
      throw error;
    }
  },
  generateClientCredentials: async (
    ...
    try {
      ...
      loggerUtils.error(
        `authService :: generateClientCredentials by clientName ${clientName} :: ${error}`
      );
      throw error;
    }
  },
  generateToken: async (
    ...
    try {
      const ttl: number = CACHE_TTL.ONE_HOUR;
  ...
  deleteClientById: async (clientId: string) => {
    try {
      await mongoUtils.updateDocuments<IClient>(
        ClientModel,
        {
          clientId,
        },
      ...
      loggerUtils.error(
        `authService :: deleteClient by clientId ${clientId} :: ${error}`
      );
      throw error;
    }
  },
  getClientById: async (clientId: string): Promise<IClient[]> => {
    try {
      const clientDetails: IClient[] =
        await mongoUtils.findDocumentsWithOptions<IClient>(
          ClientModel,
          {
            clientId,
            isActive: CLIENT_STATUS.ACTIVE,
      ...
      loggerUtils.error(
        `authService :: getClientByClientId :: clientId ${clientId} :: ${error}`
      );
      throw error;
    }
  },
      ...
      await mongoUtils.updateDocuments<IClient>(
        ...
        },
        {
          ...
          sessionConcurrencyLimit:
            clientDetails.sessionConcurrencyLimit || undefined,
