...
import { IAgent, IImage, IParticipant, ISession, SessionDetails } from "../../types/custom";
...
export const sessionService = {
  createSession: async (sessionDetails: SessionDetails, sessionExists: boolean) => {
    try {
      if (!sessionExists) {
        ...
        if (agents.length > 0) {
          const availableAgents: IAgent[] = await agentsService.getLeastActiveAgent(sessionDetails.clientId);
          if (availableAgents.length == 0) {
            loggerUtils.error(`sessionService :: createSession :: no agents are available`)
            throw Error;
          }
          
          const availableAgent = availableAgents[0];
          const baseUrl = `${availableAgent.sslEnabled ? 'https' : 'http'}://${availableAgent.agentHost}:${availableAgent.agentPort}`
          await proxyService.createProxy(sessionDetails.sessionId, sessionDetails.imageId, baseUrl);

          sessionDetails.agentId = availableAgent.agentId;
        } else {
          loggerUtils.error(`sessionService :: createSession :: no available agents are available while creating session`)
          throw Error;
        }

        const images = await imagesService.getImageByName(sessionDetails.imageName);

        if (images.length == 0) {
          loggerUtils.error(`sessionService :: createSession :: no images are available while creating session`)
          throw Error;
        } 
        ...
        const sessionObj: Partial<ISession> = sessionDetails;
        await mongoUtils.insertDocument<ISession>(SessionModel, sessionObj)
      }

      const agents: IAgent[] = await agentsService.getAgentBySessionId(sessionDetails.sessionId);
      if (agents.length == 0) {
        loggerUtils.error(`sessionService :: createSession :: no agents are available while joining session`)
        throw Error;
      }
      ...
      const participantObj: Partial<IParticipant> = {
        ...
        role: sessionExists ? PARTICIPANT_ROLES.PARTICIPANT : PARTICIPANT_ROLES.MODERATOR,
        access: sessionDetails.participantsAccess
      }

      await mongoUtils.insertDocument<IParticipant>(ParticipantModel, participantObj);

      const clientBaseUrl = getStringEnvVariableOrDefault("WORKSPACESCLIENT_BASE_URL", "http://localhost:3000")
  ...
  sessionExistsById: async (sessionId: string) => {
    try {
      ...
      loggerUtils.error(`sessionsService :: sessionExistsById by sessionId ${sessionId} :: ${error}`);
      throw error;
    }
  },
  getSessionById: async (sessionId: string): Promise<ISession[]> => {
    try {
      ...
      loggerUtils.error(`sessionsService :: getSessionById :: sessionId ${sessionId} :: ${error}`);
      throw error;
    }
  },
  listSessionsByClientId: async (clientId: string): Promise<ISession[]> => {
    try {
      ...
      loggerUtils.error(`sessionsService :: listSessionsByClientId :: clientId ${clientId} :: ${error}`);
      throw error;
    }
  },
  destroySession: async (sessionId: string, deletePersistence: boolean) => {
    try {
      const agents: IAgent[] = await agentsService.getAgentBySessionId(sessionId);
      if (agents.length == 0) {
        loggerUtils.error(`sessionService :: createSession :: no agents are available while joining session`)
        throw Error;
      }
      ...
      const agent: IAgent = agents[0];
      const baseUrl = `${agent.sslEnabled ? 'https' : 'http'}://${agent.agentHost}:${agent.agentPort}`

      await proxyService.destroyProxy(sessionId, deletePersistence, baseUrl);
    } catch (error) {
      loggerUtils.error(`sessionsService :: destroySession :: sessionId ${sessionId} :: ${error}`);
      throw error;
    }
  },
  setPermissions: async (participantId: string, access: string) => {
    try {
      await mongoUtils.updateDocuments<IParticipant>(ParticipantModel, {
        participantId
      }, {
        access
      });
      ...
      loggerUtils.error(`sessionsService :: setPermissions :: participantId ${participantId} :: access ${access} :: ${error}`);
      throw error;
    }
  },
  ...
  participantExistsById: async (participantId: string): Promise<boolean> => {
    try {
      ...
      loggerUtils.error(`sessionsService :: participantExistsById :: participantId ${participantId} :: ${error}`);
      throw error;
    }
  },
  getParticipantById: async (participantId: string): Promise<IParticipant[]> => {
    try {
      ...
      loggerUtils.error(`sessionsService :: getParticipantById :: participantId ${participantId} :: ${error}`);
      throw error;
    }
  },
  getSessionsCountByClientId: async (clientId: string): Promise<number> => {
    try {
      const sessionsCount: number = await mongoUtils.countDocuments<ISession>(SessionModel, {
        clientId,
        status: SESSIONS_STATUS.ACTIVE
      });
      return sessionsCount;
    } catch (error) {
      loggerUtils.error(`sessionsService :: getSessionsCountByClientId :: clientId ${clientId} :: ${error}`);
      throw error;
    }
  },
};