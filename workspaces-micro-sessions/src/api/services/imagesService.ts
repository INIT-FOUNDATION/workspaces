...
export const imagesService = {
  createImage: async (imageDetails: ImageDetails) => {
    try {
      ...
      loggerUtils.error(`imagesService :: createImage :: imageObj ${imageDetails} :: ${error}`);
      throw error;
    }
  },
  getImageById: async (imageId: string): Promise<IImage[]> => {
    try {
      ...
      loggerUtils.error(`imagesService :: getImageById :: imageId ${imageId} :: ${error}`);
      throw error;
    }
  },
  imageExistsById: async (imageId: string): Promise<boolean> => {
    try {
      ...
      loggerUtils.error(`imagesService :: imageExistsById :: imageId ${imageId} :: ${error}`);
      throw error;
    }
  },
  imageExistsByName: async (imageName: string): Promise<boolean> => {
    try {
      const exists: boolean = await mongoUtils.existsDocument<IImage>(ImageModel, {
        imageName,
        isActive: IMAGES_STATUS.ACTIVE
      ...
      loggerUtils.error(`imagesService :: imageExistsById :: imageName ${imageName} :: ${error}`);
      throw error;
    }
  },
  ...
  updateImageById: async (imageDetails: ImageDetails) => {
    try {
      await mongoUtils.updateDocuments<IImage>(ImageModel, {
      ...
      loggerUtils.error(`imagesService :: updateImageById :: image Id ${imageDetails.imageId} :: ${error}`);
      throw error;
    }
  },
  deleteImageById: async (imageId: string) => {
    try {
      ...
      loggerUtils.error(`imagesService :: deleteImageById :: image Id ${imageId} :: ${error}`);
      throw error;
    }
  },
  getImageByName: async (imageName: string) => {
    try {
      const images: IImage[] = await mongoUtils.findDocumentsWithOptions<IImage>(ImageModel, {
        imageName,
        isActive: IMAGES_STATUS.ACTIVE
      ...
      loggerUtils.error(`imagesService :: getImageByName :: imageName ${imageName} :: ${error}`);
      throw error;
    }
  },
};