...
import { ERROR_MESSAGES, HTTP_STATUS_CODES, jwtUtils, loggerUtils, redisUtils } from "workspaces-micro-commons";
      ...
      jwt.verify(
        ...
        async (error: jwt.VerifyErrors | null, decoded: any) => {
          ...
          if (moment().isAfter(moment(decoded.accessExpiryDate)))
            return res
              .status(HTTP_STATUS_CODES.FORBIDDEN)
              .send({ data: null, message: "Access Expired, Please renew to continue!" });


          const sessionsCount: number = await sessionService.getSessionsCountByClientId(decoded.clientId);

          if (sessionsCount == decoded.sessionsLimit)
            return res
              .status(HTTP_STATUS_CODES.FORBIDDEN)
              .send({ data: null, message: "Sessions Limit Exceeded, Please increase to continue!" });
